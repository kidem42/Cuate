...
nonisolated enum Diagnostics {
    ...
    static var logsDirectory: URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("AISpotlight/Logs", isDirectory: true)
    ...
    static func exportToDownloads() throws -> URL {
        ...
        let staging = fm.temporaryDirectory.appendingPathComponent("AISpotlight-logs-\(stamp)", isDirectory: true)
        ...
        where item.lastPathComponent.localizedCaseInsensitiveContains("aispotlight") {
        ...
        let zip = downloads.appendingPathComponent("AISpotlight-logs-\(stamp).zip")
...
nonisolated private final class DiagnosticsCore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "kpn.AISpotlight.diagnostics", qos: .utility)
    private let osLog = Logger(subsystem: "kpn.AISpotlight", category: "diagnostics")
