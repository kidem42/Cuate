...
struct HermesSettingsView: View {
    ...
    /// (usage-anchored where the install has the anchor, upgrading an
    /// older v3 fill line in place) —
    ...
    /// Recognizes Hermes up to 0.21.0 (one usage entry per line, the
    /// disconnect call closing on its own line) and 0.21.1 (packed usage
    /// dict, hugged bracket); any other layout is refused untouched.
    /// Mirrors `HermesGatewayPatch` (the local, contract-tested edits) —
    ...
    # context_tokens = real fill: preferably the usage anchor (the last
    ...
    # falling back to last_prompt_tokens on older installs. context_window =
    # the window the agent ACTUALLY operates with (OAuth caps included) —
    # the pair the client gauge needs, from the same frame. An older v3
    # fill line upgrades in place; native upstream fields are left alone.
