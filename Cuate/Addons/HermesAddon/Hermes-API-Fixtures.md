...
- `usage` в `run.completed` — СУММА ПО ВСЕМ API-вызовам рана, не размер промпта
  (снято живьём 2026-07-30: ход без тулзов → `input_tokens:17915`; тот же контекст,
  2 тул-колла → `35990`). Для гейджа контекста НЕ годится — ход с 26 шагами «занял» 2188K
  при окне 1050K. Скрытых полей нет: полный состав — `input/output/total_tokens` + `runtime{...}`.
- `usage.context_tokens` — НАШ carried-патч гейтвея (api_server.py, оба usage-словаря
  `_run_agent`): `agent.context_compressor.last_prompt_tokens` — промпт ПОСЛЕДНЕГО вызова,
  та же цифра, что в статус-баре самого Hermes. Это и есть заполнение контекста; сток-гейтвей
  (VPS) поля не шлёт → клиент падает на суммарный фолбэк с капом по окну.
