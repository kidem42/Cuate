...
- The corpus itself contains wrong-layout artifacts ("рш" = "hi" typed on a RU
  keyboard) frequent enough to reach the top-30k; listing them as "known words"
  blocked the very fixes users want (рш→hi lacked frequency dominance).
  `WRONG_LAYOUT_GARBAGE` in the gen script prunes the curated offenders
  (criterion: spellchecker-invalid, flip is a more frequent valid word of the
  other language, and removal verifiably changes a verdict; real tokens like
  ts/tls/kfc/dj/фиш stay). Harness after pruning: рш→hi fixed on both paths,
  zero new false flips across both 30k lists.
...
### Retroactive single-letter fix

Lone letters are guarded by strict frequency bounds (a standalone e/c/d must
survive), so "у владимира" typed in the EN layout used to end up as
"e владимира" — the "e" alone could never flip. When a word gets corrected,
the word before it is re-examined: if it was a single letter typed in the same
wrong layout whose rendering in the winner layout is a top-frequency function
word (у, в, с, а, i …), both are fixed together. Letters that are themselves
top-frequency words of the source language (я, и, i, a) keep their protection.
The context cache is invalidated on any contiguity break (mouse click,
navigation, shortcut, Backspace past the word start, separators, Enter) and
expires after 4 s.
