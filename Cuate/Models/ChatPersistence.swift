...
extension SDMessage {
    /// Resolves a stored recording path. Audio is kept as an ABSOLUTE URL
    /// (unlike attachments, which moved to relative paths), so every row
    /// written before the rename points into `Application Support/AISpotlight`.
    /// The recordings themselves moved with the rest of the data, so those
    /// paths are re-anchored on the current base directory.
    ///
    /// Not cosmetic: the orphan sweep matches live files against these paths,
    /// and stale ones make every referenced recording look unreferenced —
    /// which deletes the lot one hour after the update.
    static func resolveAudioURL(_ stored: String) -> URL? {
        guard let url = URL(string: stored) else { return nil }
        let parts = url.pathComponents
        guard let legacy = parts.lastIndex(of: "AISpotlight") else { return url }
        return parts[(legacy + 1)...].reduce(ChatStore.baseDirectory) {
...
nonisolated enum ChatPersistence {
    ...
    static let container: ModelContainer = {
        ...
        let url = ChatStore.baseDirectory.appendingPathComponent("CuateChats.store")
    ...
    private static let queue = DispatchQueue(label: "Cuate.ChatPersistence", qos: .utility)
    ...
    static func sweepOrphanedMedia() {
        queue.async {
            ...
            if let all = try? ctx.fetch(FetchDescriptor<SDMessage>()) {
                for row in all {
                    if let s = row.audioURLString, let url = SDMessage.resolveAudioURL(s) { referenced.insert(url.path) }
                    for attachment in row.attachments {
