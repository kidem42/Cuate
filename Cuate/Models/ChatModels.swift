...
class ChatStore: ObservableObject {
    ...
    nonisolated static var baseDirectory: URL {
        // Before the directory is created, not after: an empty `Cuate/` here
        // would make the pre-rename carry-over think it had already run.
        LegacyRenameMigration.runIfNeeded()
        let base: URL
        if let override = ProcessInfo.processInfo.environment["CUATE_DATA_DIR"], !override.isEmpty {
            ...
            base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
                .appendingPathComponent("Cuate", isDirectory: true)
        }
