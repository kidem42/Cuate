...
enum Localization {
    ...
    static let strings: [String: [AppLanguage: String]] = [
        ...
        "or.browserOpen": [.english: "Browse catalog…", .spanish: "Explorar catálogo…", .russian: "Каталог моделей…"],
        "or.browserTitle": [.english: "OpenRouter models", .spanish: "Modelos de OpenRouter", .russian: "Модели OpenRouter"],
        "or.search": [.english: "Search by name, slug or description", .spanish: "Buscar por nombre, slug o descripción", .russian: "Поиск по имени, слагу или описанию"],
        "or.filter.vision": [.english: "Images", .spanish: "Imágenes", .russian: "Картинки"],
        "or.filter.files": [.english: "Documents", .spanish: "Documentos", .russian: "Документы"],
        "or.filter.tools": [.english: "Tools", .spanish: "Herramientas", .russian: "Инструменты"],
        "or.filter.reasoning": [.english: "Reasoning", .spanish: "Razonamiento", .russian: "Размышление"],
        "or.filter.free": [.english: "Free", .spanish: "Gratis", .russian: "Бесплатные"],
        "or.sort.newest": [.english: "Newest", .spanish: "Más nuevos", .russian: "Новые"],
        "or.sort.cheapest": [.english: "Cheapest", .spanish: "Más baratos", .russian: "Дешёвые"],
        "or.sort.name": [.english: "Name", .spanish: "Nombre", .russian: "По имени"],
        "or.refresh": [.english: "Refresh", .spanish: "Actualizar", .russian: "Обновить"],
        "or.refreshHelp": [.english: "Reload the model catalog from OpenRouter now.", .spanish: "Recargar ahora el catálogo de modelos de OpenRouter.", .russian: "Перезагрузить каталог моделей с OpenRouter сейчас."],
        "or.updatedAt": [.english: "Catalog updated %@", .spanish: "Catálogo actualizado %@", .russian: "Каталог обновлён %@"],
        "or.select": [.english: "Select", .spanish: "Seleccionar", .russian: "Выбрать"],
        "or.count": [.english: "%d models", .spanish: "%d modelos", .russian: "%d моделей"],
        "or.per1M": [.english: "per 1M tokens, in / out", .spanish: "por 1M de tokens, entrada / salida", .russian: "за 1M токенов, вход / выход"],
        "or.context": [.english: "Context", .spanish: "Contexto", .russian: "Контекст"],
        "or.maxOutput": [.english: "Max output", .spanish: "Salida máx.", .russian: "Максимум ответа"],
        "or.added": [.english: "Added", .spanish: "Añadido", .russian: "Добавлена"],
        "or.openPage": [.english: "Open on openrouter.ai ↗", .spanish: "Abrir en openrouter.ai ↗", .russian: "Открыть на openrouter.ai ↗"],
        "or.noDescription": [.english: "No description in the catalog.", .spanish: "Sin descripción en el catálogo.", .russian: "В каталоге нет описания."],
        "or.pickHint": [.english: "Pick a model on the left to see its details.", .spanish: "Elige un modelo a la izquierda para ver sus detalles.", .russian: "Выберите модель слева, чтобы увидеть подробности."],
        "or.tools.search": [.english: "Web search via OpenRouter", .spanish: "Búsqueda web vía OpenRouter", .russian: "Веб-поиск через OpenRouter"],
        "or.tools.searchHelp": [.english: "The model searches through OpenRouter's server tool instead of Brave — no Brave key needed. About $0.007 per search on OpenRouter's balance.", .spanish: "El modelo busca a través de la herramienta de servidor de OpenRouter en lugar de Brave — sin clave de Brave. Unos $0.007 por búsqueda del saldo de OpenRouter.", .russian: "Модель ищет через серверный инструмент OpenRouter вместо Brave — ключ Brave не нужен. Около $0,007 за поиск с баланса OpenRouter."],
        "or.tools.fetch": [.english: "Page reading via OpenRouter", .spanish: "Lectura de páginas vía OpenRouter", .russian: "Чтение страниц через OpenRouter"],
        "or.tools.fetchHelp": [.english: "Pages the model wants to read are fetched by OpenRouter's free engine instead of this Mac.", .spanish: "Las páginas que el modelo quiere leer las descarga el motor gratuito de OpenRouter en lugar de este Mac.", .russian: "Страницы, которые модель хочет прочитать, скачивает бесплатный движок OpenRouter вместо этого Mac."],
        "or.tools.note": [.english: "Only on OpenRouter chats, only with web access on. Search results and pages pass through third parties and are outside OpenRouter's zero-data-retention setting.", .spanish: "Solo en chats de OpenRouter y con el acceso web activado. Los resultados y las páginas pasan por terceros y quedan fuera del ajuste de retención cero de OpenRouter.", .russian: "Только в чатах OpenRouter и только при включённом доступе в интернет. Результаты поиска и страницы проходят через третьи стороны и не покрываются настройкой нулевого хранения OpenRouter."],
        "or.copy": [.english: "Copy", .spanish: "Copiar", .russian: "Копировать"],
        "or.zdrOnly": [.english: "Only zero-data-retention endpoints", .spanish: "Solo endpoints sin retención de datos", .russian: "Только эндпоинты без хранения данных (ZDR)"],
        "or.zdrOnlyHelp": [.english: "Every OpenRouter request is routed only to endpoints that keep nothing, whatever the account's privacy page says. Narrows the model choice; a model with no such endpoint fails instead of falling back.", .spanish: "Cada petición a OpenRouter se enruta solo a endpoints que no guardan nada, diga lo que diga la página de privacidad de la cuenta. Reduce la elección de modelos; un modelo sin tal endpoint falla en vez de usar otro.", .russian: "Каждый запрос к OpenRouter идёт только на эндпоинты, которые ничего не хранят, независимо от страницы приватности аккаунта. Сужает выбор моделей: модель без такого эндпоинта отвечает ошибкой, а не уходит к другому провайдеру."],
        "or.filter.allowed": [.english: "Available to me", .spanish: "Disponibles para mí", .russian: "Доступные мне"],
        "or.notAllowed": [.english: "In the catalog, but your OpenRouter privacy settings leave this model no endpoint — the request will fail. Pick another model or change the settings on openrouter.ai.", .spanish: "Está en el catálogo, pero tus ajustes de privacidad de OpenRouter no le dejan ningún endpoint — la petición fallará. Elige otro modelo o cambia los ajustes en openrouter.ai.", .russian: "Есть в каталоге, но настройки приватности твоего аккаунта OpenRouter не оставляют этой модели ни одного эндпоинта — запрос упадёт. Выбери другую модель или измени настройки на openrouter.ai."],
        "or.notAllowedDetail": [.english: "No endpoint under your OpenRouter privacy settings.", .spanish: "Sin endpoint con tus ajustes de privacidad de OpenRouter.", .russian: "Нет эндпоинта под настройки приватности твоего OpenRouter."],
        "or.routingHint": [.english: "OpenRouter found no endpoint for this model under the current restrictions (your privacy settings on openrouter.ai, the no-data-collection rule for documents, or the ZDR toggle in Settings). Pick another model in the catalog browser, or relax a restriction.", .spanish: "OpenRouter no encontró ningún endpoint para este modelo con las restricciones actuales (tus ajustes de privacidad en openrouter.ai, la regla sin recogida de datos para documentos o el interruptor ZDR en Ajustes). Elige otro modelo en el explorador del catálogo o relaja una restricción.", .russian: "OpenRouter не нашёл эндпоинт для этой модели под текущими ограничениями (настройки приватности на openrouter.ai, запрет сбора данных для документов или тумблер ZDR в настройках). Выбери другую модель в каталоге или ослабь ограничение."],
        ...
        "cap.documents.native": [.english: "Documents: PDF, Word, Excel, PowerPoint and text files — read natively by the provider (PDF with page images, the rest as text). Up to 3 per message, 50 MB.", .spanish: "Documentos: PDF, Word, Excel, PowerPoint y texto — el proveedor los lee de forma nativa (PDF con imágenes de página, el resto como texto). Hasta 3 por mensaje, 50 MB.", .russian: "Документы: PDF, Word, Excel, PowerPoint и текст — провайдер читает их нативно (PDF с картинками страниц, остальное как текст). До 3 на сообщение, 50 MB."],
        "cap.documents.text": [.english: "Documents: PDF, Word and text files as text extracted on this Mac (scans via Apple OCR; no page images, so charts inside a PDF are not visible). Up to 3 per message, 50 MB.", .spanish: "Documentos: PDF, Word y texto como texto extraído en este Mac (escaneos con OCR de Apple; sin imágenes de página, los gráficos de un PDF no se ven). Hasta 3 por mensaje, 50 MB.", .russian: "Документы: PDF, Word и текст как текст, извлечённый на этом Mac (сканы через Apple OCR; без картинок страниц, графики внутри PDF не видны). До 3 на сообщение, 50 MB."],
        "cap.documents.openrouter": [.english: "Documents: a PDF goes to the model as a file on models that list file input (see the model's chips), otherwise as text extracted by OpenRouter's free parser; Word and text files as text from this Mac. Every turn with a document is routed only to providers that don't collect data. Up to 3 per message, 20 MB as a file.", .spanish: "Documentos: un PDF llega al modelo como archivo en los modelos que aceptan archivos (mira las etiquetas del modelo), si no como texto extraído por el analizador gratuito de OpenRouter; Word y texto como texto desde este Mac. Cada turno con un documento se enruta solo a proveedores que no recogen datos. Hasta 3 por mensaje, 20 MB como archivo.", .russian: "Документы: PDF уходит модели файлом у моделей с поддержкой файлов (см. метки модели), иначе текстом через бесплатный парсер OpenRouter; Word и текст как текст с этого Mac. Каждый ход с документом маршрутизируется только к провайдерам, которые не собирают данные. До 3 на сообщение, 20 MB файлом."],
        "cap.documents": [.english: "Documents", .spanish: "Documentos", .russian: "Документы"],
        "cap.images.yes": [.english: "Images: sent as pixels (vision).", .spanish: "Imágenes: se envían como píxeles (visión).", .russian: "Картинки: отправляются пикселями (vision)."],
        "cap.images.ocr": [.english: "Images: no vision — each image is OCR'd into text.", .spanish: "Imágenes: sin visión — cada imagen se convierte en texto por OCR.", .russian: "Картинки: без vision — каждая распознаётся в текст через OCR."],
        "cap.tools.yes": [.english: "Tools: web search, calendar, documents.", .spanish: "Herramientas: búsqueda web, calendario, documentos.", .russian: "Инструменты: веб-поиск, календарь, документы."],
        "cap.tools.perModel": [.english: "Tools: depends on the model.", .spanish: "Herramientas: según el modelo.", .russian: "Инструменты: зависит от модели."],
        ...
        "dictation.silence.title": [.english: "Dictation heard nothing", .spanish: "El dictado no oyó nada", .russian: "Диктовка ничего не услышала"],
        "dictation.silence.body": [.english: "The microphone delivered pure silence — the input device is probably wrong (a Bluetooth headset often grabs the input without its mic working). Pick a microphone in System Settings → Sound → Input, or pin one in Cuate Settings → Voice.", .spanish: "El micrófono entregó silencio absoluto — el dispositivo de entrada probablemente es incorrecto (unos auriculares Bluetooth suelen tomar la entrada sin que su micrófono funcione). Elige un micrófono en Ajustes del Sistema → Sonido → Entrada, o fija uno en Ajustes de Cuate → Voz.", .russian: "Микрофон отдал чистую тишину — скорее всего, выбрано не то устройство входа (Bluetooth-наушники часто перехватывают вход, а их микрофон не работает). Выберите микрофон в Системных настройках → Звук → Вход или закрепите его в настройках Cuate → Голос."],
        ...
        "dictation.cleanup.provider": [.english: "Cleanup provider", .spanish: "Proveedor de limpieza", .russian: "Провайдер очистки"],
        "dictation.cleanup.model": [.english: "Cleanup model", .spanish: "Modelo de limpieza", .russian: "Модель очистки"],
        "dictation.cleanup.caption": [.english: "A second model, after transcription: it removes fillers the speech provider leaves in, fixes false starts and punctuation, and does the translation. A small, fast model is the right choice — it runs once per phrase.", .spanish: "Un segundo modelo, tras la transcripción: quita las muletillas que deja el proveedor de voz, corrige falsos inicios y puntuación, y hace la traducción. Un modelo pequeño y rápido es lo adecuado — se ejecuta una vez por frase.", .russian: "Вторая модель, уже после распознавания: убирает слова-паразиты, которые оставил провайдер речи, чинит ложные старты и пунктуацию, делает перевод. Нужна маленькая и быстрая — она работает на каждую фразу."],
        "dictation.cleanup.unavailable": [.english: "No provider with an API key — the transcript is inserted as recognized.", .spanish: "Ningún proveedor con clave API — el texto se inserta tal cual se reconoció.", .russian: "Нет провайдера с ключом API — текст вставится как распознан."],
        ...
        "dictation.streaming": [.english: "Allow streaming (live transcription)", .spanish: "Permitir streaming (transcripción en vivo)", .russian: "Разрешить стриминг (распознавание на лету)"],
        ...
        "switcher.dragHelp": [.english: "Drag to reorder", .spanish: "Arrastra para reordenar", .russian: "Потяните, чтобы изменить порядок"],
        "switcher.orderFooter": [.english: "Drag a row by its name to reorder the presets. The order applies everywhere: the panel menu, the button row (the first five presets shown in the panel get buttons, the rest move into “…”), and the preset picker on the Prompts tab.", .spanish: "Arrastra una fila por su nombre para reordenar los preajustes. El orden se aplica en todas partes: el menú del panel, la fila de botones (los primeros cinco preajustes visibles en el panel reciben botón, el resto pasa a «…») y el selector de preajustes en la pestaña Prompts.", .russian: "Потяните строку за название, чтобы изменить порядок пресетов. Порядок действует везде: меню в панели, ряд кнопок (кнопки получают первые пять показанных в панели пресетов, остальные уходят в «…») и выбор пресета на вкладке промптов."],
        ...
        "chatfiles.title": [.english: "Chat files", .spanish: "Archivos del chat", .russian: "Файлы чата"],
        "chatfiles.empty": [.english: "No files exchanged in this chat yet.", .spanish: "Aún no hay archivos en este chat.", .russian: "В этом чате пока нет файлов."],
        "chatfiles.model": [.english: "Created by the model", .spanish: "Creados por el modelo", .russian: "Создано моделью"],
        "chatfiles.user": [.english: "Attached by you", .spanish: "Adjuntados por ti", .russian: "Прикреплено вами"],
        "chatfiles.help": [.english: "All files in this chat", .spanish: "Todos los archivos de este chat", .russian: "Все файлы этого чата"],
        "chatfiles.open": [.english: "Open", .spanish: "Abrir", .russian: "Открыть"],
        "chatfiles.reveal": [.english: "Reveal in Finder", .spanish: "Mostrar en Finder", .russian: "Показать в Finder"],
        ...
        "panel.attachCountMixed": [.english: "%d of %d attachments", .spanish: "%d de %d adjuntos", .russian: "%d из %d вложений"],
        "panel.docLimit": [.english: "You can attach up to %d documents to one message.", .spanish: "Puedes adjuntar hasta %d documentos a un mensaje.", .russian: "К одному сообщению можно прикрепить не больше %d документов."],
        "panel.docTooLarge": [.english: "%@ is larger than %@ and can't be sent.", .spanish: "%@ supera %@ y no se puede enviar.", .russian: "%@ больше %@, отправить нельзя."],
        "panel.docMessageTooLarge": [.english: "Documents in one message must total under %@.", .spanish: "Los documentos de un mensaje deben sumar menos de %@.", .russian: "Документы в одном сообщении вместе должны быть меньше %@."],
        "panel.docUnsupported": [.english: "%@: this file type isn't supported.", .spanish: "%@: este tipo de archivo no es compatible.", .russian: "%@: этот тип файла не поддерживается."],
        "panel.docEmpty": [.english: "%@ is empty.", .spanish: "%@ está vacío.", .russian: "%@ пустой."],
        "panel.docEncrypted": [.english: "%@ is password-protected and can't be read.", .spanish: "%@ está protegido con contraseña y no se puede leer.", .russian: "%@ защищён паролем, прочитать нельзя."],
        "panel.docLarge": [.english: "Large document — high token cost", .spanish: "Documento grande — alto coste en tokens", .russian: "Большой документ — дорого по токенам"],
        "panel.docPages": [.english: "%d pages", .spanish: "%d páginas", .russian: "%d стр."],
        "panel.uploadingDoc": [.english: "Uploading %@…", .spanish: "Subiendo %@…", .russian: "Загружаю %@…"],
        "panel.readingDoc": [.english: "Reading %@…", .spanish: "Leyendo %@…", .russian: "Читаю %@…"],
        "panel.docSentAsText": [.english: "%@ was sent as text — the upload failed: %@", .spanish: "%@ se envió como texto — falló la subida: %@", .russian: "%@ отправлен текстом — загрузка не удалась: %@"],
        "panel.memoryDocDays": [.english: "%d d", .spanish: "%d d", .russian: "%d дн."],
        "chatfiles.attachAgain": [.english: "Attach to the next message again — no re-upload.", .spanish: "Adjuntar al siguiente mensaje de nuevo — sin volver a subirlo.", .russian: "Снова приложить к следующему сообщению — без повторной загрузки."],
        ...
        "tooltip.dictation.streaming": [.english: "Transcribe over a live Deepgram connection: phrases are inserted while you speak, and the stop only flushes the last words. Billed at Deepgram's streaming rate; falls back to a normal upload if the stream breaks.", .spanish: "Transcribe por una conexión en vivo de Deepgram: las frases se insertan mientras hablas y el stop solo vacía las últimas palabras. Se factura a la tarifa de streaming de Deepgram; si la conexión falla, se usa la subida normal.", .russian: "Распознавание по живому соединению с Deepgram: фразы вставляются прямо во время речи, стоп лишь дожимает последние слова. Тарифицируется по стриминговой ставке Deepgram; при обрыве потока — обычная загрузка записи."],
        ...
        "ob.tour.chat.body": [
            .english: "Cuate lives in the menu bar and opens over any app. Ask, read the answer, press Esc.",
            .spanish: "Cuate vive en la barra de menús y se abre sobre cualquier app. Pregunta, lee la respuesta y pulsa Esc.",
            .russian: "Cuate живёт в строке меню и открывается поверх любого приложения. Спросили — получили ответ — Esc."
        ],
        "ob.tour.shot.title": [
