...
- **Файлы от агента**: упомянутые пути становятся чипами (тап = копия — хост
  удалённый); ⋮ → «Файлы этого чата» собирает все пути беседы + группу
  «Отправленные вами»; HTML/Markdown-артефакты рендерятся штатной карточкой.
