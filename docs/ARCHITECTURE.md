...
| CalendarAddon | `Addons/CalendarAddon` | tab, `CalendarToolService` in `ChatService`, EventKit access request on enable | calendar/reminder tools | this map + `docs/addon-tool-playbook.md` |
| WorldTimeAddon | `Addons/WorldTimeAddon` | own panel + hotkey + status-menu item, tab; uses CalendarAddon for the busy lane and `WorldTimeSlotService` (EventKit, no LLM) for slot events | none | this map |
