...
| VoiceMessagePlayer / EnhancedVoiceButton / RecordingStatusView / DictationService | voice, the mic, the recording pill, the dictation island (a black tab 34 tall, 14 bars in `dictationColors`, the glow and the chip from the DARK palette only — `recordingAccent` → `quoteColor` → `accent`; ornaments by `themeID`: Yule = `CandyCaneSpinner` 74×4 as the processing line, Día = `DiaIslandPennants` / `DiaIslandMarigolds` / `DiaIslandPetals` + the tricolor runner) |
