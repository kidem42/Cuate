...
**Назначение:** пошаговый чек-лист для добавления нового чат-провайдера в AISpotlight — от enum'а до вкладки «Расходы». Порядок шагов = порядок зависимостей; после каждого блока указано, чем проверять.

Все пути — относительно `AISpotlight/AISpotlight/`.
